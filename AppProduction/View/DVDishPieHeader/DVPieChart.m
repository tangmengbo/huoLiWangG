...
-(void)drawRect:(CGRect)rect{
    ...
    CGFloat radius = min * self.radiusBiLi.floatValue - CHART_MARGIN;
    CGFloat start = 0;
    CGFloat angle = 0;
    CGFloat end = start;
...
//        NSMutableArray * huXianArrayY1 = [NSMutableArray array];
//        NSMutableArray * huXianArrayY2 = [NSMutableArray array];
        ...
        for (int i = 0; i < self.dataArray.count; i++) {
            ...
            if (i <= self.dataArray.count / 2 - 1) {
                ...
                [self addLineWithColor:color X:lineStartX Y:lineStartY name:model.name number:@"" radianCenter:radianCenter];
    ...
    centerView.frame = CGRectMake(0, 0, 70*BILI, 70*BILI);
...
- (void)addLineWithColor:(UIColor *)color X:(CGFloat)x Y:(CGFloat)y name:(NSString *)name number:(NSString *)number radianCenter:(CGFloat)radianCenter {
    ...
    if (self.guaiDianX) {
        ...
        breakPointX =   x + self.guaiDianX.floatValue * cos(radianCenter);
    }
    else
    {
        breakPointX =  x + 10 * cos(radianCenter);
    }
    
    CGFloat breakPointY = y + self.guaiDianY.floatValue * sin(radianCenter);
    
//    if (breakPointY>0) {
//        
//        breakPointY = breakPointY-50*BILI;
//    }
//    else
//    {
//        breakPointY = breakPointY+50*BILI;
//    }
    ...
    // 数字的frame
    CGFloat numberWidth = 90.f;
    if(self.lineXWidth)
    {
        numberWidth = self.lineXWidth.floatValue;
    }
    CGFloat numberHeight = 15.f;
    if (self.fontNumber) {
        
        numberHeight = self.fontNumber.floatValue;
    }
   ...
   // [self addSubview:view];
    ...
    //指引线上面的数字
    [name drawInRect:CGRectMake(numberX, numberY+3, numberWidth, numberHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:numberHeight-5], NSForegroundColorAttributeName:strColor,NSParagraphStyleAttributeName:paragraph}];
    
    // 指引线下面的title
    [number drawInRect:CGRectMake(titleX, titleY, titleWidth, titleHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:numberHeight-5],NSForegroundColorAttributeName:strColor,NSParagraphStyleAttributeName:paragraph}];
}
...
- (void)drawLineWithPointArray:(NSArray *)pointArray centerArray:(NSArray *)centerArray  {
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        if (model.value!=1) {
            ...
            //指引线上面的数字
            [name drawInRect:CGRectMake(numberX, numberY+1, numberWidth, numberHeight) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:9.0], NSForegroundColorAttributeName:color,NSParagraphStyleAttributeName:paragraph}];
