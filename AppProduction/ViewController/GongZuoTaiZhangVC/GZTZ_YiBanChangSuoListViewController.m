//
//  GZTZ_YiBanChangSuoListViewController.m
//  SeeYou
//
...
#import "GZTZ_YiBanChangSuoListViewController.h"

@interface GZTZ_YiBanChangSuoListViewController ()

@end

@implementation GZTZ_YiBanChangSuoListViewController
...
- (void)viewDidLoad {
    ...
    self.titleLale.text = @"一般场所巡查台账";
    //[self.searchBar setPlaceholder:@"请输入场所名称查询"];// 搜索框的占位符
    self.searchBar.delegate = self;
    [self.searchBar changeLeftPlaceholder:@"请输入场所名称查询"];

    [self showNewLoadingView:nil view:self.view];
    [self.cloudClient yiBanChangSuoXunChaList:@"patrolVisits!ybPatrolList.do"
                                  pagesize:@"20"
                                    pageno:@"1"
...
{
    pageIndex = 1;
    [self.cloudClient ruHuZouFangXinXiList:@"patrolVisits!ybPatrolList.do"
                                  pagesize:@"20"
                                    pageno:@"1"
                                  keywords:self.searchBar.text
                                  delegate:self
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        ...
        ZhongDianAndYiBanChangSuoListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:tableIdentifier];
        if (cell == nil)
        {
            cell = [[ZhongDianAndYiBanChangSuoListTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:tableIdentifier];
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        ...
        ZhongDianAndYiBanChangSuoDetailViewController * vc = [[ZhongDianAndYiBanChangSuoDetailViewController alloc] init];
        vc.patrolid = [info objectForKey:@"patrolid"];
        vc.fromWhere = @"yiBanChangSuo";
