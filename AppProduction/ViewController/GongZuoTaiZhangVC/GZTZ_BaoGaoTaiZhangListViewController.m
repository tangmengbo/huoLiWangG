//
//  GZTZ_BaoGaoTaiZhangListViewController.m
//  SeeYou
//
...
#import "GZTZ_BaoGaoTaiZhangListViewController.h"

@interface GZTZ_BaoGaoTaiZhangListViewController ()

@end

@implementation GZTZ_BaoGaoTaiZhangListViewController
...
- (void)viewDidLoad {
    ...
    self.titleLale.text = @"报告台账";
    //[self.searchBar setPlaceholder:@"请输入事件名称名查询"];// 搜索框的占位符
    self.searchBar.delegate = self;
    [self.searchBar changeLeftPlaceholder:@"请输入事件名称名查询"];

    [self showNewLoadingView:nil view:self.view];
    [self.cloudClient chuZhiAndBaoGaoTaiZhangList:@"eventInfo.do"
                                         pagesize:@"20"
                                           pageno:@"1"
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {//
        ...
        ChuZhiAndBaoGaoListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:tableIdentifier];
        if (cell == nil)
        {
            cell = [[ChuZhiAndBaoGaoListTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:tableIdentifier];
        }
