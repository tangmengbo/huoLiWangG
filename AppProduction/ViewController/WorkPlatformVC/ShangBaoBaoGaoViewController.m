//
//  ShangBaoBaoGaoViewController.m
//  SeeYou
//
...
#import "ShangBaoBaoGaoViewController.h"

@interface ShangBaoBaoGaoViewController ()

@end

@implementation ShangBaoBaoGaoViewController
...
- (void)viewDidLoad {
    ...
    self.neiRongTextView.zw_placeHolder = @"描述事件内容...";
    ...
    UILabel * addressLable = [[UILabel alloc] initWithFrame:CGRectMake(13*BILI,neiRongLineView.frame.origin.y+1, 75*BILI, 60*BILI)];
...
-(void)zhanKaiButtonClick
{
    ...
    if (self.zhanKaiShouQiButton.tag==0) {
        ...
        if(self.contentView.frame.origin.y+self.contentView.frame.size.height>self.mainScrollView.frame.size.height+10)
        {
...
-(void)initImageContentView
{
    ...
    if(self.contentView.frame.origin.y+self.contentView.frame.size.height+100*BILI>self.mainScrollView.frame.size.height+10)
    {
...
-(void)tiJiaoButtonClick
{
    ...
    {
        [Common showToastView:@"请填事件内容" view:self.view];
        return;
    }
    ...
    {
        NSDictionary * dataDic = [Common getNowDateAndWeek];
        [self.cloudClient wangGeShiJianShangBao:@"eventInfo!eventReportAdd.do"
                                         gridid:self.grid
...
-(void)tiJiaoSuccess:(NSDictionary *)info
{
    ...
    [self.delegate shagBaoBaoGaoSuccess];
